...
class LeadGeneratorWorker:
    ...
    async def run(self, run_id: str, source: str = "all") -> Dict[str, Any]:
        ...
        try:
            ...
            # Track leads created in this run
            leads_before = set()
            try:
                existing = lead_db.get_all_leads()
                leads_before = {str(l['id']) for l in existing}
            except Exception:
                pass
            ...
            new_leads = [l for l in all_leads if str(l['id']) not in leads_before]
